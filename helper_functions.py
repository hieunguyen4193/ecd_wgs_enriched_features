...
def run_analysis(all_input_samples, path_to_save_output, show_plot = False):
    ...
    for input_sample in all_input_samples:
        tmp_flendf = pd.read_csv([item for item in pathlib.Path(os.path.join(input_sample, "fragmentomics_features")).glob("*GWfeature_FLEN*")][0])[["size", "freq"]].copy()
        tmp_flendf.columns = ["size", input_sample.split("/")[-1].split("_")[0].split("-")[1]]
    ...
    for input_sample in all_input_samples:
        tmp_emdf = pd.read_csv([item for item in pathlib.Path(os.path.join(input_sample, "fragmentomics_features")).glob("*GWfeature_EM*")][0])[["motif", "freq"]].copy()
        tmp_emdf.columns = ["motif", input_sample.split("/")[-1].split("_")[0].split("-")[1]]
        emdf = emdf.merge(tmp_emdf, on="motif", how="left")
    emdf = emdf.fillna(0)
    nddf = pd.DataFrame(data = range(-300, 301), columns = ["dist"])
    for input_sample in all_input_samples:
        tmp_nddf = pd.read_csv([item for item in pathlib.Path(os.path.join(input_sample, "fragmentomics_features")).glob("*GWfeature_ND*")][0])[["dist", "freq"]].copy()
        tmp_nddf.columns = ["dist", input_sample.split("/")[-1].split("_")[0].split("-")[1]]
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_output, "fragment_size_distribution.pdf"))
    # plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_output, "nucleosome_distance_distribution.pdf"))
    # plt.show()
    ...
    os.system(f"mkdir -p { os.path.join(path_to_save_output, 't-test-results') }")

    flen_ttest_df.to_excel(os.path.join(path_to_save_output, "t-test-results", "FLEN.xlsx"), index=False)
    em_ttest_df.to_excel(os.path.join(path_to_save_output, "t-test-results", "EM.xlsx"), index=False)
    nd_ttest_df.to_excel(os.path.join(path_to_save_output, "t-test-results", "ND.xlsx"), index=False)

    # %%
    os.system(f"mkdir -p {os.path.join(path_to_save_output, 'diff_end_motifs') }")
    plt.figure(figsize=(6, 4))

    for motif in em_ttest_df[em_ttest_df["p_adj"] <= 0.05].motif.unique():
        if os.path.isfile(os.path.join(path_to_save_output, "diff_end_motifs", f"motif_{motif}_frequency.pdf")) == False:
            ...
            plt.ylabel("Frequency")
            plt.tight_layout()
            plt.savefig(os.path.join(path_to_save_output, "diff_end_motifs", f"motif_{motif}_frequency.pdf"))
            # plt.show()
    ...
    def plot_pca(df, feature_col, sample_list, sample_labels, title, savename):
        os.system(f"mkdir -p {os.path.join(path_to_save_output, 'PCA')}")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(path_to_save_output, "PCA", f"{savename}.pdf"))
        # plt.show()
        return X_pca
    ...
    def calculate_ot_distance(sample1, sample2, inputdf, n):
        if inputdf[inputdf[sample1] != 0].shape[0] == 0 or inputdf[inputdf[sample2] != 0].shape[0] == 0:
            return np.nan 
    ...
    os.system(f"mkdir -p {os.path.join(path_to_save_output, 'MDS') } ")
    # Prepare distance matrix for MDS
    dist_matrix = flen_distdf[sample_list].values
    if np.isnan(dist_matrix).any() == False:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(path_to_save_output, "MDS", f"MDS_from_flen_dist.pdf"))
    ...
    if np.isnan(dist_matrix).any() == False:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(path_to_save_output, "MDS", f"MDS_from_em_dist.pdf"))
        # plt.show()
    ...
    if np.isnan(dist_matrix).any() == False:
        mds = MDS(n_components=2, dissimilarity='precomputed', random_state=42)
        print("Any NaN in dist_matrix:", np.isnan(dist_matrix).any())
        mds_coords = mds.fit_transform(dist_matrix)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(path_to_save_output, "MDS", f"MDS_from_nd_dist.pdf"))
    ...
    avgdist.to_excel(os.path.join(path_to_save_output, "avg_OT_dist.xlsx"), index = False)
    ...
    avgdist["feature"] = ["flen", "em", "nd"]
    avgdist.to_excel(os.path.join(path_to_save_output, "avg_barycenter_OT_dist.xlsx"), index = False)
    ...
    entropydf.to_excel(os.path.join(path_to_save_output, "fragment_length_entropy.xlsx"), index = False)
    abs_diff_entropy = abs(entropydf[entropydf["Label"] == "Control"]["entropy"].mean() - entropydf[entropydf["Label"] == "Cancer"]["entropy"].mean())
    plt.figure()
    sns.boxplot(data = entropydf, x = "Label", y = "entropy", palette={"Control": "lightgray", "Cancer": "red"})
    plt.title("Fragment Length Entropy by Sample label, abs diff entropy = %.4f" % abs_diff_entropy)
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_output, "fragment_length_entropy.pdf"))
    # plt.show()
    ...
    entropydf = entropydf.merge(metadata, right_on = "LABCODE", left_on = "LABCODE")
    entropydf.to_excel(os.path.join(path_to_save_output, "end_motif_entropy.xlsx"), index = False)
    abs_diff_entropy = abs(entropydf[entropydf["Label"] == "Control"]["entropy"].mean() - entropydf[entropydf["Label"] == "Cancer"]["entropy"].mean())

    sns.boxplot(data = entropydf, x = "Label", y = "entropy", palette={"Control": "lightgray", "Cancer": "red"})
    plt.title("End Motif Entropy by Sample label, abs diff entropy = %.4f" % abs_diff_entropy)
    plt.tight_layout()
    plt.savefig(os.path.join(path_to_save_output, "end_motif_entropy.pdf"))
    # plt.show()
    ...
    pd.DataFrame(data = ["finished"], columns=["status"]).to_csv(os.path.join(path_to_save_output, "status.txt"), index = False)
